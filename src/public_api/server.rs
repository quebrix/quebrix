// use actix_web::{http::{self, header::{self, HeaderMap}}, middleware::Logger, web, App,HttpRequest, HttpResponse, HttpServer};
// use serde::{Deserialize, Serialize};
// use std::sync::{Arc, Mutex};
// use crate::{cache::Cache, creds::cred_manager::{CredsManager, RoleManagement}};
...
//     match creds.lock().unwrap().add_user(username.clone(), password.clone(),role.parse::<RoleManagement>()) {
...
//     let creds:Vec<&str> = decoded_credentials.split(":").collect();
//     let username = creds.get(0).unwrap();
//     let password = creds.get(1).unwrap();
