...
use crate::creds::load_user_from_file::LoadUserFromFile;
...
pub async fn load_users_from_file(
    ...
    let decoded_credentials = std::str::from_utf8(&decoded_bytes).unwrap();
    ...
    let result = cred.load_users_from_file();
    if result.is_success {
        return HttpResponse::Ok().json(ApiResponse::ok(result.clone().message));
    } else {
        return HttpResponse::Ok().json(ApiResponse::fail(result.clone().message));
    }
}
